...
\[
\lambda=x_2 - x_1 = 120\mathrm{mm} - 60\mathrm{mm} =60\ \mathrm{mm}=0{,}060\ \mathrm{m}.
\]
...
El espectro electromagnético comprende el conjunto de todas las ondas
electromagnéticas, clasificadas de acuerdo con su frecuencia $f$ o su
longitud de onda $\lambda$. Ambas magnitudes se relacionan mediante:

\begin{equation}
    c = \lambda f
\end{equation}

donde $c \approx 3 \times 10^{8}\ \text{m/s}$ es la velocidad de propagación
de la luz en el vacío. Por lo tanto, al aumentar la frecuencia disminuye la
longitud de onda, y viceversa.

El espectro electromagnético se divide, de manera general, en las siguientes
regiones: ondas de radio, microondas, infrarrojo, luz visible, ultravioleta,
rayos X y rayos gamma. Dentro de la región correspondiente a las
radiofrecuencias, la Unión Internacional de Telecomunicaciones (ITU) establece
distintas bandas de frecuencia.

Entre las principales bandas utilizadas en telecomunicaciones se encuentran:

\begin{table}[H]
\centering
\begin{tabular}{c c c}
\hline
\textbf{Banda} & \textbf{Nombre} & \textbf{Rango de frecuencia} \\
\hline
VLF & Very Low Frequency & $3 - 30\ \text{kHz}$ \\
LF  & Low Frequency & $30 - 300\ \text{kHz}$ \\
MF  & Medium Frequency & $300\ \text{kHz} - 3\ \text{MHz}$ \\
HF  & High Frequency & $3 - 30\ \text{MHz}$ \\
VHF & Very High Frequency & $30 - 300\ \text{MHz}$ \\
UHF & Ultra High Frequency & $300\ \text{MHz} - 3\ \text{GHz}$ \\
SHF & Super High Frequency & $3 - 30\ \text{GHz}$ \\
EHF & Extremely High Frequency & $30 - 300\ \text{GHz}$ \\
\hline
\end{tabular}
\caption{Principales bandas de radiofrecuencia según la nomenclatura de la ITU.}
\end{table}

Para la onda analizada se obtuvo una longitud de onda es de \( \lambda = 0.06\ \text{m} \), y teniendo en
cuenta que la frecuencia calculada es de \( f \approx 5\ \text{GHz} \), podemos determinar que se encuentra
dentro de la región de \textbf{microondas} del espectro electromagnético. Además, según la
clasificación de la ITU, se encuentra dentro de la banda
\textbf{SHF (Super High Frequency)}, cuyo rango se extiende desde
$3\ \text{GHz}$ hasta $30\ \text{GHz}$.

Por lo tanto, la onda estudiada puede clasificarse como:

\begin{equation}
    \boxed{f = 5\ \text{GHz}
    \quad \Rightarrow \quad
    \text{Microondas} \quad \Rightarrow \quad \text{SHF}}
\end{equation}
...
El gráfico del enunciado muestra una portadora senoidal cuya frecuencia y
amplitud se mantienen constantes, pero cuya fase varía según el valor del bit.
La técnica es \textbf{BPSK} (\emph{Binary Phase Shift Keying}): la información
se codifica en la \emph{fase absoluta} de la portadora. Al bit~0 se le asigna
una fase de $0°$ y al bit~1 una fase de $180°$ (inversión de la portadora),
manteniendo constantes la amplitud y la frecuencia.
...
Se asigna una fase de $0°$ al bit~0 y una fase de $180°$ al bit~1. La
representación conceptual resultante aparece en la Figura~\ref{fig:bpsk}.
...
\begin{tikzpicture}[x=1.25cm,y=1.15cm]
  ...
  \foreach \x in {0,...,8}{
    \draw[gray!45] (\x,-0.75) -- (\x,1.10);
    \node[font=\tiny, below] at (\x,-0.75) {$T_{\x}$};
  }
  % Etiquetas de bits
  \foreach \x/\b in {0/0,1/1,2/1,3/1,4/0,5/1,6/1,7/0}{
    \node[font=\bfseries] at (\x+0.5,1.27) {\b};
  }
  % Señal BPSK: misma frecuencia, fase 0° (bit 0) o 180° (bit 1)
  % Bit 0 -> sin(720*(x-start))   Bit 1 -> -sin(720*(x-start))
  \draw[very thick,blue!70!black,domain=0:1,samples=50]   % bit 0, fase 0°
    plot (\x,{0.55*sin(720*\x)});
  \draw[very thick,blue!70!black,domain=1:2,samples=50]   % bit 1, fase 180°
    plot (\x,{-0.55*sin(720*(\x-1))});
  \draw[very thick,blue!70!black,domain=2:3,samples=50]   % bit 1, fase 180°
    plot (\x,{-0.55*sin(720*(\x-2))});
  \draw[very thick,blue!70!black,domain=3:4,samples=50]   % bit 1, fase 180°
    plot (\x,{-0.55*sin(720*(\x-3))});
  \draw[very thick,blue!70!black,domain=4:5,samples=50]   % bit 0, fase 0°
    plot (\x,{0.55*sin(720*(\x-4))});
  \draw[very thick,blue!70!black,domain=5:6,samples=50]   % bit 1, fase 180°
    plot (\x,{-0.55*sin(720*(\x-5))});
  \draw[very thick,blue!70!black,domain=6:7,samples=50]   % bit 1, fase 180°
    plot (\x,{-0.55*sin(720*(\x-6))});
  \draw[very thick,blue!70!black,domain=7:8,samples=50]   % bit 0, fase 0°
    plot (\x,{0.55*sin(720*(\x-7))});
  % Etiquetas de fase debajo de cada bit
  \node[font=\small,below=0.90cm] at (0.5,0) {$0^\circ$};
  \node[font=\small,below=0.90cm] at (1.5,0) {$180^\circ$};
  \node[font=\small,below=0.90cm] at (2.5,0) {$180^\circ$};
  \node[font=\small,below=0.90cm] at (3.5,0) {$180^\circ$};
  \node[font=\small,below=0.90cm] at (4.5,0) {$0^\circ$};
  \node[font=\small,below=0.90cm] at (5.5,0) {$180^\circ$};
  \node[font=\small,below=0.90cm] at (6.5,0) {$180^\circ$};
  \node[font=\small,below=0.90cm] at (7.5,0) {$0^\circ$};
\end{tikzpicture}
\caption{Representación BPSK de la secuencia $01110110$: misma frecuencia,
fase $0^\circ$ para el bit~0 y fase $180^\circ$ para el bit~1.}
\label{fig:bpsk}
\end{figure}
